...
glm::vec3 parse_vector3(const std::string& str)
{
    ...
    std::regex vec_regex(R"(Vector3\s*\(\s*([-\d.]+)\s*,\s*([-\d.]+)\s*,\s*([-\d.]+)\s*\))");
    std::regex simple_regex(R"(([-\d.]+)\s*,\s*([-\d.]+)\s*,\s*([-\d.]+))");
    std::smatch match;
    
    if (std::regex_search(str, match, vec_regex) || std::regex_search(str, match, simple_regex))
    {
...
bool parse_transform3d(const std::string& str, glm::vec3& pos, glm::vec3& rot, glm::vec3& scale)
{
    // Try to extract 12 floats from Transform3D(...)
    std::regex transform_regex(R"(Transform3D\s*\(\s*([-\d.]+)\s*,\s*([-\d.]+)\s*,\s*([-\d.]+)\s*,\s*([-\d.]+)\s*,\s*([-\d.]+)\s*,\s*([-\d.]+)\s*,\s*([-\d.]+)\s*,\s*([-\d.]+)\s*,\s*([-\d.]+)\s*,\s*([-\d.]+)\s*,\s*([-\d.]+)\s*,\s*([-\d.]+)\s*\))");
    std::smatch match;
    ...
    if (std::regex_search(str, match, transform_regex))
    {
        ...
        if (scale.x > 0.0001f) { m00 /= scale.x; m10 /= scale.x; m20 /= scale.x; }
        if (scale.y > 0.0001f) { m01 /= scale.y; m11 /= scale.y; m21 /= scale.y; }
        if (scale.z > 0.0001f) { m02 /= scale.z; m12 /= scale.z; m22 /= scale.z; }
...
{
    std::regex res_regex(R"delim((?:Ext|Sub)Resource\s*\(\s*"([^"]+)"\s*\))delim");
    std::smatch match;
...
bool load_tscn(const std::string& tscn_path)
{
    ...
    if (!tscn_fs_path.is_absolute())
    {
        try
        {
            ...
            spdlog::debug("Resolved relative path to absolute: {}", tscn_fs_path.string());
        }
        catch (const std::exception& e)
        {
            spdlog::error("Failed to resolve absolute path for {}: {}", tscn_path, e.what());
    ...
    {
        spdlog::error("TSCN path is not a regular file: {}", tscn_fs_path.string());
    ...
    if (!file.is_open())
    {
        spdlog::error("Failed to open TSCN file for reading: {}", tscn_fs_path.string());
    ...
    while (std::getline(file, line))
    {
        ...
        if (line[0] == '[' && line.back() == ']')
        {
            ...
            {
                // Header with attributes like [node name="Node3D" type="Node3D"]
                ...
                if (current_section == "node")
                {
                    ...
                    std::regex parent_regex(R"delim(parent\s*=\s*"([^"]+)")delim");
                    std::smatch match;
                    ...
                    spdlog::debug("Parsed node header: name='{}', type='{}', parent='{}'", 
                                 current_node.name, current_node.type, current_node.parent);
                }
                else if (current_section == "ext_resource")
                {
                    ...
                    // Parse attributes: type="..." path="..." id="..." uid="..."
                    // Note: id= must be matched carefully to not match uid=
                    std::string attrs = header_content.substr(space_pos + 1);
                    std::regex type_regex(R"delim(type\s*=\s*"([^"]+)")delim");
                    std::regex path_regex(R"delim(path\s*=\s*"([^"]+)")delim");
                    // Match id= but NOT uid= (use word boundary or space before)
                    std::regex id_regex(R"delim((?:^|\s)id\s*=\s*"([^"]+)")delim");
                    std::smatch match;
                    ...
                    if (std::regex_search(attrs, match, path_regex))
                    {
                        ...
                        auto resolved = resolve_resource_path(path_val, tscn_dir);
                        current_resource.path = resolved.string();
                        spdlog::debug("ext_resource path resolved: '{}' -> '{}'", path_val, current_resource.path);
                    }
                    if (std::regex_search(attrs, match, id_regex))
                    {
                        current_resource.id = match[1].str();
                        spdlog::debug("ext_resource id: '{}'", current_resource.id);
                    }
                    
                    if (!current_resource.id.empty())
                    {
                        ext_resources[current_resource.id] = current_resource;
                        spdlog::info("Stored ext_resource: id='{}', type='{}', path='{}'", 
                                    current_resource.id, current_resource.type, current_resource.path);
                    }
                }
                else if (current_section == "sub_resource")
                {
                    ...
                    {
                        current_resource.id = match[1].str();
                        spdlog::debug("sub_resource id: '{}'", current_resource.id);
                    }
                    ...
                    if (!current_resource.id.empty())
                    {
                        ...
                        spdlog::info("Stored sub_resource: id='{}', type='{}'", 
                                    current_resource.id, current_resource.type);
                    }
                }
            }
            
            if (current_section == "ext_resource" && space_pos == std::string::npos)
            {
                if (!current_resource.id.empty())
                    ext_resources[current_resource.id] = current_resource;
                current_resource = tscn_resource();
            }
            else if (current_section == "sub_resource" && space_pos == std::string::npos)
            {
                if (!current_resource.id.empty())
                    sub_resources[current_resource.id] = current_resource;
                current_resource = tscn_resource();
            }
            else if (current_section == "node" && space_pos == std::string::npos)
            {
                if (!current_node.name.empty())
                    nodes.push_back(current_node);
                current_node = tscn_node();
    ...
    spdlog::info("Parsed TSCN: {} ext_resources, {} sub_resources, {} nodes", 
                ext_resources.size(), sub_resources.size(), nodes.size());
    ...
    {
        spdlog::info("ExtResource: id='{}', type='{}', path='{}'", id, res.type, res.path);
    }
    ...
    for (const auto& node : nodes)
    {
        spdlog::debug("Checking node: type='{}', name='{}'", node.type, node.name);
        if (node.type != "MeshInstance3D") 
        {
            spdlog::debug("Skipping node '{}' (type: '{}')", node.name, node.type);
            continue;
        }
        mesh_instance_count++;
        ...
        spdlog::info("Node '{}' has {} properties:", node.name, node.properties.size());
        ...
        {
            spdlog::error("MeshInstance3D '{}' has no mesh property! Available properties:", node.name);
            for (const auto& [k, v] : node.properties)
            {
                spdlog::error("  '{}' = '{}'", k, v);
            }
        ...
        {
            spdlog::error("MeshInstance3D '{}' has invalid mesh resource ID from: '{}'", node.name, mesh_it->second);
            continue;
        }
        
        spdlog::info("MeshInstance3D '{}' references mesh resource ID: '{}'", node.name, mesh_res_id);
        
        // Look up resource
        spdlog::info("Looking for resource ID '{}' in {} ext_resources, {} sub_resources", 
                    mesh_res_id, ext_resources.size(), sub_resources.size());
        ...
        if (ext_it != ext_resources.end())
        {
            ...
            spdlog::info("Found mesh in ext_resource '{}': path='{}'", mesh_res_id, mesh_path);
        }
        else
        {
            spdlog::info("Not found in ext_resources, checking sub_resources...");
            auto sub_it = sub_resources.find(mesh_res_id);
            if (sub_it != sub_resources.end())
            {
                spdlog::info("Found mesh in sub_resource '{}' (type: {})", mesh_res_id, sub_it->second.type);
                ...
                {
                    auto resolved = resolve_resource_path(path_it->second, tscn_dir);
                    mesh_path = resolved.string();
                    spdlog::info("Resolved sub_resource path: {}", mesh_path);
                }
                else
                {
                    spdlog::error("sub_resource '{}' has no resource_path property", mesh_res_id);
                }
            }
            else
            {
                spdlog::error("Mesh resource ID '{}' not found in ext_resources or sub_resources!", mesh_res_id);
        ...
        if (mesh_path.empty())
        {
            spdlog::error("MeshInstance3D '{}' has empty mesh path, skipping", node.name);
            continue;
        }
        ...
        else
        {
            ...
            if (rot_it != node.properties.end())
            {
                ...
                rot = rot_rad * 180.0f / glm::pi<float>(); // Convert radians to degrees
            }
        ...
        // Engine: Y-up, +Z toward camera, -X right (left-handed vs right-handed)
        // Negate both X and Z to convert coordinate systems
        pos.x = -pos.x;
        pos.z = -pos.z;
        ...
        if (avg_scale < 0.001f) avg_scale = 1.0f;
        ...
        if (auto* m = scene::add_model(mesh_path, pos, avg_scale))
        {
            ...
            loaded_count++;
            spdlog::info("Loaded model '{}' from mesh: {}", node.name, mesh_path);
        }
        else
        {
            spdlog::error("Failed to load model '{}' from mesh: {}", node.name, mesh_path);
        }
    }
    
    spdlog::info("TSCN parsing complete: {} MeshInstance3D nodes found, {} successfully loaded", 
                mesh_instance_count, loaded_count);
    
    if (loaded_count > 0)
    {
        spdlog::info("Successfully loaded {} objects from TSCN: {}", loaded_count, tscn_fs_path.filename().string());
        ui::log(2, std::format("Loaded {} objects from TSCN: {}", loaded_count, tscn_fs_path.filename().string()));
    }
    else
    {
        spdlog::warn("No objects loaded from TSCN file: {} (found {} total nodes, {} MeshInstance3D nodes)", 
                    tscn_fs_path.filename().string(), nodes.size(), mesh_instance_count);
        ui::log(3, std::format("No objects loaded from TSCN: {} (found {} MeshInstance3D nodes)", 
                              tscn_fs_path.filename().string(), mesh_instance_count));
    }
