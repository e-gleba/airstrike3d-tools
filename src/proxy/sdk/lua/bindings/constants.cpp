/// @file constants.cpp
/// @brief Lua bindings for Windows virtual key codes and OpenGL constants.
...
/// @brief Helper to bulk-register name/value pairs into a table.
...
{
    // -- Virtual keys --
    auto vk{state.create_named_table("VK")};
    ...
    // A-Z
    for (const char c : std::views::iota('A') | std::views::take(26))
    {
        vk[std::string(1, c)] = static_cast<int>(c);
    }

    // 0-9
    for (const char c : std::views::iota('0') | std::views::take(10))
    {
        vk[std::string(1, c)] = static_cast<int>(c);
    }

    // F1-F24
    for (const int i : std::views::iota(1) | std::views::take(24))
    {
