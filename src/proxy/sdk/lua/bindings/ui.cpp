...
bool drag_float(const std::string& label, float& v, float spd, float mn, float mx) noexcept
{
    return ImGui::DragFloat(label.c_str(), &v, spd, mn, mx);
}

bool slider_float(const std::string& label, float& v, float mn, float mx) noexcept
{
...
bool color_edit3(const std::string& label, float& r, float& g, float& b) noexcept
{
    std::array col{r, g, b};
...
{
    return ImGui::CollapsingHeader(label.c_str(), open ? ImGuiTreeNodeFlags_DefaultOpen : 0);
}
...
{
    ImGui::ProgressBar(fraction, {-1, 0}, overlay.empty() ? nullptr : overlay.c_str());
}
