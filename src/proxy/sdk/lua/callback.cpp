...
void callback_list::add_on_glu_lookat(
    std::function<bool(double, double, double, double, double, double, double, double, double)> fn)
{
    std::lock_guard lk{ mtx };
...
{
    std::lock_guard lk{ mtx };
    std::ranges::for_each(on_frame_fns, [](auto& fn) { fn(); });
}

void callback_list::invoke_on_overlay()
{
    std::lock_guard lk{ mtx };
    std::ranges::for_each(on_overlay_fns, [](auto& fn) { fn(); });
}

void callback_list::invoke_on_gl_identity()
{
    std::lock_guard lk{ mtx };
    std::ranges::for_each(on_gl_identity_fns, [](auto& fn) { fn(); });
}

void callback_list::invoke_on_load()
{
    std::lock_guard lk{ mtx };
    std::ranges::for_each(on_load_fns, [](auto& fn) { fn(); });
}

void callback_list::invoke_on_unload()
{
    std::lock_guard lk{ mtx };
    std::ranges::for_each(on_unload_fns, [](auto& fn) { fn(); });
}

bool callback_list::invoke_on_glu_lookat(double ex, double ey, double ez,
                                          double cx, double cy, double cz,
                                          double ux, double uy, double uz)
{
    std::lock_guard lk{ mtx };
    return std::ranges::any_of(on_glu_lookat_fns,
...
{
    std::lock_guard lk{ mtx };
    return std::ranges::any_of(on_key_down_fns, [&](auto& fn) { return fn(vk); });
}
