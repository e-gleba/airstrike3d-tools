...
/// Counts vtable entries by scanning for null or invalid addresses.
/// COM vtables typically have 90+ entries for IDirect3DDevice8.
[[nodiscard]] constexpr auto count_vtable_entries(void** vtable) noexcept
    -> uint32_t
{
    // IDirect3DDevice8 has ~98 vtable entries; we allocate conservatively.
    // The exact count is determined by the runtime; 128 is a safe upper bound.
    return 128;
}
...
void install_device_hooks(IDirect3DDevice8* dev)
{
    ...
    const auto entry_count = count_vtable_entries(g_device_vtable);
