...
public:
    ...
    [[nodiscard]] SDL_GPUTexture* depth_texture() const { return depth_texture_; }

    // IRenderer interface
    void set_view_projection(const glm::mat4& vp) override;
    void set_render_mode(render_mode mode) override;
    [[nodiscard]] render_mode get_render_mode() const override { return render_mode_; }
    ...
    [[nodiscard]] bool pipeline_valid() const { return wireframe_pipeline_ != nullptr; }

private:
    bool create_wireframe_pipeline();
    bool create_textured_pipeline();
    void draw_mesh_internal(const gpu_mesh& mesh);
    void draw_textured_mesh_internal(const gpu_textured_mesh& mesh, texture_handle tex);
    gpu_mesh upload_wireframe_mesh(std::span<const vertex_pos_color> vertices, std::span<const uint16_t> indices);
    gpu_textured_mesh upload_textured_mesh(std::span<const vertex_textured> vertices, std::span<const uint16_t> indices);
    
    // Find texture file for model
    std::filesystem::path find_texture_for_model(const std::filesystem::path& model_path);
    ...
    SDL_GPUGraphicsPipeline* textured_wireframe_pipeline_ = nullptr;  // Textured shader with wireframe fill
