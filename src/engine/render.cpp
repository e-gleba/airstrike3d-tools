...
void Renderer::begin_frame(SDL_GPUCommandBuffer* cmd, SDL_GPURenderPass* pass)
{
    current_cmd_                 = cmd;
    current_pass_                = pass;
    frame_stats_                 = {}; // Reset per-frame stats
    frame_stats_.models_loaded   = static_cast<uint32_t>(models_.size());
    frame_stats_.textures_loaded = static_cast<uint32_t>(textures_.size());
    frame_stats_.meshes_loaded   = static_cast<uint32_t>(meshes_.size());
    reload_pipelines();
}
                                            ...
                                            const glm::vec3& color)
{
    ...
    for (auto& o : k_cube_offsets)
        verts.push_back({ center + o * half, color });
    for (auto& [i, j] : k_cube_edges)
    {
        idx.push_back(static_cast<uint16_t>(i));
        idx.push_back(static_cast<uint16_t>(j));
    }
                                              ...
                                              int              seg)
{
    ...
    auto                          circle = [&](int a1, int a2)
    {
        ...
        {
            float ang =
                6.28318f * static_cast<float>(i) / static_cast<float>(seg);
            glm::vec3 p = center;
            p[a1] += radius * std::cos(ang);
            p[a2] += radius * std::sin(ang);
            verts.push_back({ p, color });
            if (i > 0)
            {
                idx.push_back(static_cast<uint16_t>(base + i - 1));
                idx.push_back(static_cast<uint16_t>(base + i));
            }
        }
    };
    circle(0, 1);
                                            ...
                                            const glm::vec3& color)
{
    ...
    for (int i = 0; i <= div; ++i)
    {
        ...
        verts.push_back({ { half, 0, t }, color });
        idx.push_back(b);
        idx.push_back(static_cast<uint16_t>(b + 1));
        idx.push_back(static_cast<uint16_t>(b + 2));
        idx.push_back(static_cast<uint16_t>(b + 3));
    }
    meshes_[next_mesh_handle_] = upload_wireframe_mesh(verts, idx);
    return next_mesh_handle_++;
}
                                  ...
                                  [[maybe_unused]] primitive_type)
{
    ...
    for (size_t i = 0; i < verts.size(); ++i)
        v[i] = { verts[i].position, verts[i].color };
                                  ...
                                  const glm::vec3&             color)
{
    ...
    for (const auto& shape : shapes)
    {
        ...
        for (size_t f = 0; f < shape.mesh.num_face_vertices.size(); ++f)
        {
            ...
            {
                auto i =
                    shape.mesh.indices[index_offset + static_cast<size_t>(v)];

                // Validate indices
                if (i.vertex_index < 0 ||
                    static_cast<size_t>(i.vertex_index * 3 + 2) >=
                ...
                vert.position = glm::vec3(
                    attrib
                        .vertices[static_cast<size_t>(3 * i.vertex_index + 0)],
                    attrib
                        .vertices[static_cast<size_t>(3 * i.vertex_index + 1)],
                    attrib
                        .vertices[static_cast<size_t>(3 * i.vertex_index + 2)]);

                if (i.normal_index >= 0 &&
                    static_cast<size_t>(i.normal_index * 3 + 2) <
                        attrib.normals.size())
                    vert.normal = glm::vec3(attrib.normals[static_cast<size_t>(
                                                3 * i.normal_index + 0)],
                                            attrib.normals[static_cast<size_t>(
                                                3 * i.normal_index + 1)],
                                            attrib.normals[static_cast<size_t>(
                                                3 * i.normal_index + 2)]);
                else
                ...
                if (i.texcoord_index >= 0 &&
                    static_cast<size_t>(i.texcoord_index * 2 + 1) <
                        attrib.texcoords.size())
                    vert.texcoord =
                        glm::vec2(attrib.texcoords[static_cast<size_t>(
                                      2 * i.texcoord_index + 0)],
                                  attrib.texcoords[static_cast<size_t>(
                                      2 * i.texcoord_index + 1)]);
...
render_stats Renderer::get_stats() const
{
    return frame_stats_;
}
